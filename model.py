...
class PositionalEmbedding(nn.Module):
    def __init__(self,d_model:int,seq_len:int, dropout:float)->None:
        ...
        # Matrice of (seq_len, d_model)
        pe=torch.zeros(seq_len,d_model)  
        # Vector of dim (seq_len,1)
        position=torch.arange(0,seq_len,dtype=torch.float).unsqueeze(1)
        div_term=torch.exp(torch.arange(0,d_model,2).float()*(-math.log(1000.0)/d_model))
        # Apply cos to odd and sin to even
...
class MultiHeadAttentionBlock(nn.Module):
    ...
    def forward(self,q,k,v,mask):
        ...
        x=x.transpose(1,2).reshape(x.shape[0],-1,self.h*self.d_k) #(batch,h, seq_len,d_k)-->(batch,seq_len,h,d_k)-->(batch,seq_len,d_model)
...
class EncoderBlock(nn.Module):
    def __init__(self,features: int,self_attention_block:MultiHeadAttentionBlock,feed_forward_block:FeedForwardBlock,dropout):
        super().__init__()
